...
from .responses import forbidden, bad_request, server_error, not_found
import traceback
        ...
        for category in ['Diet', 'Exercise', 'Sleep', 'Lifestyle']:
            ...
            if recs:
                ...
                for rec in recs:
                    ...
                    recommendation_text = f"- ({disease}) {clean_text}"
                    self.multi_cell(0, 5, recommendation_text, 0, 'L', False)
                    self.set_text_color(0, 0, 0)  # Reset to black
                
                self.ln(3)

@api_bp.route('/patients/<patient_id>/report/pdf', methods=['POST'])
@jwt_required()
def download_patient_report(patient_id):
    ...
    try:
        ...
        risk_prediction = RiskPrediction.objects(patient_id=patient).order_by('-predicted_at').first()
        ...
        if "Overview" in sections:
            ...
            overview_data = {
                "Name": patient.name.encode('ascii', 'ignore').decode('ascii'),
                "Age": patient.age,
                "Gender": patient.gender,
                "Height": f"{patient.height} cm",
                "Weight": f"{patient.weight} kg",
                "BMI": patient.bmi,
                "State": patient.state_name.encode('ascii', 'ignore').decode('ascii') if patient.state_name else "N/A",
...
@api_bp.route('/patients/<patient_id>/share', methods=['POST'])
@jwt_required()
