...
def seed_static_recommendations():
    """
    Populates the LifestyleRecommendation collection with static MVP data
    if it is currently empty.
    """
    try:
        if LifestyleRecommendation.objects().first() is not None:
        ...
        # In MongoEngine, we can use insert for bulk saving
        LifestyleRecommendation.objects.insert(recommendations)
        current_app.logger.info("Successfully seeded static LifestyleRecommendations in MongoDB.")
